...
def get_weighted_box(boxes, conf_type="avg"):
    ...
    if conf_type == "avg":
        ...
        box[1] = conf / len(boxes)
    box[2] = w
    # model index field is retained for consistensy but is not used.
    box[3] = -1
    box[4:] /= conf
    return box
...
def weighted_boxes_fusion(
    ...
    for label in filtered_boxes:
        ...
        for i in range(len(new_boxes)):
            ...
            # skip cluster when it does not contain enough votes, votes are measured
            # by taking the number unique models which have a prediction in the cluster
            if vote_thr > 1:
                # models predictions idx=3
                models_present = np.unique(clustered_boxes[:, 3])
            ...
            if conf_type == "box_and_model_avg":
                ...
                weighted_boxes[i][1] = (
                    weighted_boxes[i][1]
                    * min(weights.sum(), len(clustered_boxes))
                    / weights.sum()
                )
            else:
                weighted_boxes[i][1] = (
        ...
        if cluster_to_remove:
            weighted_boxes = np.delete(weighted_boxes, cluster_to_remove, axis=0)
