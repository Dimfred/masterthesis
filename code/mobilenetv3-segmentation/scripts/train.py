...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MobileNetV3 for Segmentation Training with Pytorch"
    )
    # model and dataset
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--ohem", type=bool, default=False, help="OHEM Loss for cityscapes dataset"
    )
    parser.add_argument(
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.model = get_segmentation_model(
            args.model, dataset=args.dataset, aux=args.aux, norm_layer=BatchNorm2d
        )
        if args.distributed:
        ...
        if args.ohem:
            ...
            self.criterion = MixSoftmaxCrossEntropyOHEMLoss(
                args.aux, args.aux_weight, min_kept=min_kept, ignore_index=-1
            ).to(self.device)
        else:
            self.criterion = MixSoftmaxCrossEntropyLoss(
                args.aux, args.aux_weight, ignore_index=-1
            ).to(self.device)
...
if __name__ == "__main__":
    ...
    logger = setup_logger(
        "mobilenetv3_segmentation",
        args.log_dir,
        get_rank(),
        filename="{}_{}_log.txt".format(args.model, args.dataset),
    )
