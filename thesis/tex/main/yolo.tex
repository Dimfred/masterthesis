...
% \begin{algorithm}[caption={DIoU-NMS Algorithm}, label={alg1}]
...
\SetAlgoLined
\KwIn{$B$ = {$b_1$,...,$b_N$} (list of bounding box proposals)

$S$ = {$s_1$,...,$s_N$} (list of bounding box scores)

$\epsilon$ (NMS threshold)
}

\KwOut{ $B_S = \{b_1,...,b_M\}$ (list of bounding boxes after NMS)

$S_S = \{s_1,...,s_M\}$ (list of bounding box scores after NMS)
}
...
\While{B $\neq$ empty} {
    ...
    \For{b$_i$, s$_i$ in B, S} {
        \If{DIoU(B$_m$, b$_i$) $\geq$ $\epsilon$} {
            B $\gets$ B - b$_i$;
            S $\gets$ S - s$_i$;
        }
    }
